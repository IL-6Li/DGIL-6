...
def load_bert_feature(ids, bert_path):
    """
    :param query: query id
    :param bert_path: dir saving Bert feature files
    """
    if bert_path[-1] != '/':bert_path += '/'
    res=[]
    for id in ids:
        name =id
        if id[0] =='>':name=id[1:]
        tmp=load_bert_embedding(name,bert_path)
        res.append(np.array(tmp))

    return res

def load_bert_embedding(seq_name, bert_folder):
    """
    :param seq_name: Sequence name
    :param bert_folder: Path to the folder containing BERT embeddings
    :return: List of embeddings for the sequence
    """
    if bert_folder[-1] != '/':
        bert_folder += '/'
    embedding_seqs = []
    with open(os.path.join(bert_folder, seq_name + '.txt'), 'r') as f:
        for line in f:
            values = line.strip().split()  # Split values by whitespace
            embedding_seq = [float(value) for value in values]
